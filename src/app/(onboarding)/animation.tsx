...
export default function AnimationScreen() {
  ...
  useEffect(() => {
    ...
    const t2 = setTimeout(() => {
      Animated.timing(beforeOpacity, { toValue: 0, duration: 300, useNativeDriver: true }).start();
      Animated.timing(afterOpacity, { toValue: 1, duration: 300, useNativeDriver: true }).start(() => {
        setPhase('after');
      });
    }, 2500);

    // T=4.0s: enable continue
    const t3 = setTimeout(() => setCanContinue(true), 4000);

    return () => { clearTimeout(t1); clearTimeout(t2); clearTimeout(t3); };
  }, [beforeOpacity, afterOpacity, buttonScale]);

  const handleContinue = useCallback(() => {
    router.push('/(onboarding)/sleep-wake' as any);
  }, []);

  return (
    <View style={styles.container}>
      <View style={styles.panelContainer}>
        <Animated.View style={{ opacity: beforeOpacity, position: phase === 'after' ? 'absolute' : 'relative', width: '100%' }}>
          <View>
            <Text style={[styles.panelTitle, { color: theme.colors.onSurface, marginBottom: theme.spacing.md }]}>BEFORE</Text>
        ...
        <Animated.View style={{ opacity: afterOpacity, position: phase === 'before' ? 'absolute' : 'relative', width: '100%' }}>
          <View>
            <Text style={[styles.panelTitle, { color: theme.colors.onSurface, marginBottom: theme.spacing.md }]}>AFTER</Text>
        ...
        <Animated.View style={{ transform: [{ scale: buttonScale }], alignSelf: 'center', marginTop: theme.spacing.lg }}>
      ...
      <Text style={styles.caption}>When life happens, Shift AI rebuilds your entire day in seconds.</Text>

      <Pressable
        style={({ pressed }) => [styles.continueButton, !canContinue && styles.continueDisabled, pressed && { opacity: theme.interaction.pressedOpacity }]}
